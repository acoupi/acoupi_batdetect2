...
## ---------------------------------------------------- ##
class cProfile_ProgramConfiguration(BaseModel):
...
ProgramConfig = TypeVar("ProgramConfig", bound=cProfile_ProgramConfiguration)


class cProfile_Program(AcoupiProgram[ProgramConfig]):
    ...
    def validate_dirs(self, config: ProgramConfig):
        if not self.paths.tmp_audio.exists():
            self.paths.tmp_audio.mkdir(parents=True)
        if not self.paths.recordings.exists():
            self.paths.recordings.mkdir(parents=True)
        if not self.paths.db_metadata.parent.exists():
            self.paths.db_metadata.parent.mkdir(parents=True)
    ...
    def configure_recorder(self, config: ProgramConfig) -> types.AudioRecorder:
    ...
    def configure_model(self, config: ProgramConfig):
        return BatDetect2()
    ...
    def register_detection_task(self, config: ProgramConfig):
        ...
        def detection_task_callback(recording: data.Recording):
            ...
            cprofile_detection_task = tasks.cprofile_create_detection_task(
                ...
                cprofile_output=self.paths.cprofile_detection,
    ...
    def register_messaging_task(self, config: ProgramConfig):
        ...
        cprofile_messaging_task = tasks.cprofile_create_messaging_task(
            cprofile_output=self.paths.cprofile_messaging,
    ...
    def register_file_management_task(self, config: ProgramConfig):
        file_management_task = tasks.generate_file_management_task(
            ...
            tmp_path=self.paths.tmp_audio,
        )

        cprofile_management_task = tasks.cprofile_create_management_task(
            cprofile_output=self.paths.cprofile_management,
