...
def test_audio_config_defaults_have_been_overwriten(
    microphone_config, messaging_config
):
    ...
    assert config.recording.schedule_start == datetime.time(hour=18)
    assert config.recording.schedule_end == datetime.time(hour=6)
