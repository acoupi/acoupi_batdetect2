...
def test_management_tempfiles_notprocess(
    ...
    recordings, model_outputs = program.store.get_recordings(
        ids=[temp_recording.id],
    )
...
def test_can_run_management_program(
    tmp_path: Path,
    recording: data.Recording,
    celery_app: Celery,
    celery_worker: WorkController,
    ...
    # Get the test recording in the database .
    ...
    # Check that the recording files with positive detection was moved.
    recording_file = [0][0]
    assert isinstance(detections, data.ModelOutput)
    assert len(detections.detections) > 0

    # Check that the recording files with negative detection was moved.
